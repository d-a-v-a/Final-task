...
contract Ballot {
    ...
    function giveRightToVote(address voter) external onlyChairperson onlyWhileVoting {
        require(!voters[voter].voted, "The voter already voted");
        require(voters[voter].weight == 0, "Voter weight must be zero");
        voters[voter].weight = 1;
    }

    function delegate(address to) external onlyWhileVoting {
    ...
    function revokeDelegation() external onlyWhileVoting {
        Voter storage sender = voters[msg.sender];
